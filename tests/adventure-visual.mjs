...
for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
 ...
 assert.deepEqual(state('Versetztes Paar').items.map(item=>item.lane),['upper','lower']);
 assert.deepEqual(state('Paralleles Paar').items.map(item=>item.encounterForm),['parallel','parallel']);
