...
for (const [name, engine] of [['chromium', chromium], ['webkit', webkit]]) {
  ...
  await page.evaluate(async stage => {
    ...
    window.paint = (scene, t=1, reduced=false) => { g.items=scenes[scene]||[]; g.feeding=scene==='nest'?1:0; g.player.wet=scene!=='nest'; g.player.y=scene==='nest'?285:530; g.player.feedX=scene==='nest'?175:undefined; drawWorld(canvas.getContext('2d'),g,scene==='menu'?'menu':'playing',t,'classic',[],reduced); };
    window.gallery = () => {
    ...
    window.paint('dense');
    window.measure = () => new Promise(resolve => { let start,last; const frames=[]; function tick(now) { start??=now; if(last) frames.push(now-last); last=now; window.paint('dense',(now-start)/1000); if(now-start<30000) requestAnimationFrame(tick); else { frames.sort((a,b)=>a-b); resolve({frames:frames.length,p95:frames[Math.floor(frames.length*.95)]}); } } requestAnimationFrame(tick); });
  }, stage);
  ...
  if (!process.env.POLISH_SHOTS_ONLY) results.browsers[name] = { version: browser.version(), ...await page.evaluate(() => window.measure()) };
  await browser.close();
}
